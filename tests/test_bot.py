...
class BotTest(unittest.TestCase):
    ...
    def test_free_text_flow(self):
        ...
        self.assertEqual(buttons, [])  # no action buttons — corrections go through Hermes, not Telegram taps
    ...
    def test_photo_autosave_high_confidence(self):
        """A clear read (fixture 01, confidence high, known category) is saved immediately, no buttons at all."""
        ...
        self.assertIn("Koreksi/hapus lewat Hermes ya kalau perlu", text)
    ...
    def test_autosave_confirmation_edit_failure_falls_back_to_send(self):
        ...
        self.assertIsNone(self.sent[-1][2])
    ...
    def test_photo_burst_all_get_scanned(self):
        """Several photos sent before the first is even read must all end up read — none silently dropped."""
        ocr.run = lambda img: (FIX / "01.txt").read_text().splitlines()  # confidence high → autosaves
        bot.handle_update(msg(OWNER, **PHOTO))
        bot.handle_update(msg(OWNER, **PHOTO))  # arrives while photo 1 is still queued/being read
        bot.handle_update(msg(OWNER, **PHOTO))
        self.assertEqual([t for _, t, _ in self.sent if "antrean" in t],
                         ["📥 Struk masuk antrean (nomor 2), tunggu ya ⏳", "📥 Struk masuk antrean (nomor 3), tunggu ya ⏳"])
        for t in threading.enumerate():
            if t.name == "ocr":
                t.join(5)
        self.assertEqual(len(db.list_tx("2000-01-01", "2100-01-01")), 3)  # all three were actually scanned and saved
        self.assertEqual(len(bot._photo_q), 0)
